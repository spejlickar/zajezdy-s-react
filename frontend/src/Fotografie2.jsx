...
const Fotografie = ({ index, fotky, setFotky, zajezdId }) => {
    ...
    if (index !== undefined) { if (!(fotky[index].id)) {
        console.log("fantomas ha ha");
        handleSave;}
    }
    //console.log(`fantomas chi chi${fotky[index].id}`);
    ...
    const handleFotkyChange = (e) => {
        ...
        if (files) { 
            if (index !== undefined) {   //klasická úprava fotky
                ...
                setFotky([...Array.from(files).map((file) => ({ id:undefined, file, tempUrl: URL.createObjectURL(file), tempPopis: "", zajezd: { id: zajezdId } })), ...fotky]);
            }
        }
    };
